...
if ($profile) {
    ...
    if ($profile.context -ne 196608 -or $profile.maxContext -ne 196608) { Add-Error 'Profile context must remain exactly 196608.' }
    ...
    if ($spec.enabled -ne $true -or $spec.algorithm -ne 'DSPARK' -or $spec.draftQuantization -ne 'modelopt_fp4' -or $spec.numSteps -ne 1 -or $spec.eagleTopk -ne 1 -or $spec.numDraftTokens -ne 8) {
        Add-Error 'Bundled DSpark speculative profile changed.'
    }
    if ($profile.containerName -ne 'sglang-qwen38-lmhead4') { Add-Error 'Profile container name must remain sglang-qwen38-lmhead4.' }
