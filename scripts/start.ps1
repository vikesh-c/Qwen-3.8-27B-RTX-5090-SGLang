...
# Mount the model parent dir and address the model subdirectories in-container
# (both target and draft live under models\gittensor):
#   -v C:\local-llm\models\gittensor:/models/base:ro
#     -> /models/base/Qwen3.8-27B-NVFP4-RTX5090-LMHead4
#     -> /models/base/Qwen3.8-27B-DSpark-NVFP4-eba1ac5a-fp16residual-bf16cast
$baseMount = Join-Path $LocalRoot 'models\gittensor'
$modelRel = (Get-ProfileValue -Profile $profile -Name 'modelRelativePath')
$draftRel = (Get-ProfileValue -Profile $profile -Name 'draftRelativePath')
foreach ($rel in @($modelRel, $draftRel)) {
    if ([IO.Path]::IsPathRooted($rel)) { throw "Profile path must be relative: $rel" }
}
$baseLeaf = ($modelRel -split '\\')[-1]
$draftLeaf = ($draftRel -split '\\')[-1]
$modelDir = Join-Path $baseMount $baseLeaf
$draftDir = Join-Path $baseMount $draftLeaf
...
$run = @(
    ...
    '--model-path', "/models/base/$baseLeaf"
    '--served-model-name', $modelId
    '--host', '0.0.0.0'
    '--port', [string]$port
    '--api-key', $key.Value
    '--speculative-algorithm', (Get-ProfileValue -Profile $spec -Name 'algorithm' -Fallback 'DSPARK')
    '--speculative-draft-model-path', "/models/base/$draftLeaf"
    '--speculative-draft-model-quantization', (Get-ProfileValue -Profile $spec -Name 'draftQuantization' -Fallback 'modelopt_fp4')
    '--speculative-num-steps', [string](Get-ProfileValue -Profile $spec -Name 'numSteps' -Fallback 1)
